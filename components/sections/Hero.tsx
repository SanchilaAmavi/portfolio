...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative min-h-screen overflow-hidden flex items-center pt-20 pb-10 section-hero-background">
      ...
      {/* ── Full-bleed hero image (wider on large screens) ── */}
      <div className="absolute inset-y-0 right-0 w-full lg:w-[74%] xl:w-[78%] z-[2] overflow-hidden">
        <div className="hero-image-highlight hidden lg:block" />
        <Image
          src={personalInfo.profileImage}
          alt={personalInfo.name}
          fill
          priority
          className="object-cover saturate-115 contrast-[1.08] brightness-[0.96]"
          style={{ objectPosition: "center 22%" }}
        />
        {/* stronger left-side blend for image */}
        <div className="absolute inset-0 hero-image-overlay" />
        {/* Gradient blend left */}
        <div className="absolute inset-0 bg-gradient-to-r from-[var(--bg)]/96 via-[var(--bg)]/48 to-transparent" />
        {/* Top navbar fade */}
        ...
        <div className="max-w-[700px]">
          ...
          <div className={`transition-all duration-700 delay-[100ms] ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <h1 className="leading-[0.93] tracking-tight" style={{ fontFamily: "Syne, sans-serif" }}>
              <span className="block text-[clamp(2.8rem,6vw,5rem)] font-extrabold text-[var(--text)]">
                ENGINEERING
              </span>
              <span className="block text-[clamp(2.8rem,6vw,5rem)] font-extrabold hero-heading-outline uppercase">
                INTELLIGENT
              </span>
              <span className="inline-flex items-baseline gap-3">
                <span className="block text-[clamp(2.8rem,6vw,5rem)] font-extrabold gradient-text">
                  SYSTEMS
                </span>
